...
public class RedisCacheActionFactory implements ActionFactory {
  ...
  public Action create(String alias, JsonObject config, Vertx vertx, Action doAction) {
    RedisCacheOptions options = new RedisCacheOptions(config);
    Redis redisClient = Redis
        .createClient(io.vertx.reactivex.core.Vertx.newInstance(vertx), options.getRedis());
    return new CacheAction(alias, doAction, new CacheOptions(config),
        new RedisCache(redisClient, options));
  }
}
